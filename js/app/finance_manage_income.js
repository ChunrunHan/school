define(['mui', 'mall'], function(mui, $) {
	...
	mui.plusReady(function() {
		console.log('#finance_manage_income plus ready');
		webviewInit();
		
		//	主动下拉刷新
		window.addEventListener('income', function() {
			console.log('收入刷新')
	...
	function pulldownRefresh() {
		page = 0;
		getIncomeList(page, size, false);
	}
	//	上拉加载
	function pullupRefresh() {
		page++;
		getIncomeList(page, size, true);
	}

	// 获取收入详细列表
	function getIncomeList(page, size, isUp) {
		plus.nativeUI.showWaiting();
		zoneId = plus.storage.getItem('zoneId');
		urlBaseP = plus.storage.getItem('urlBaseP');
		var url = urlBaseP + "/ywh/finance/0/list/"+ page +'/' + size;
		console.log(url);
		$.get(url).then(function(data) {
			console.log('获取收入详细列表 ' + JSON.stringify(data));
			if(data != null && data.length !== 0) {
				...
				mui.each(data,function(index,item){
					...
					switch(item.financeCategoryId){
						case 1:
							item.img = 'images/zu.svg';
							break;
						case 2:
							item.img = 'images/carzu.svg';
							break;
						case 3:
							item.img = 'images/ad.svg';
							break;
						case 4:
							item.img = 'images/qtsr.svg';
							break;
					}
