define(['mui', 'mall'], function(mui, $) {
	...
	mui.plusReady(function() {
		...
		console.log('#temporary_vehicle_warn_sub plusReady()');
		webviewInit();
		...
		//	主动下拉刷新
		window.addEventListener('showWarn', function() {
...
//		var url = urlBaseP + '/ipms/1/' + zoneId + '///////searchCar/list/' + page + '/' + pageSize;
		var url = urlBaseP + '/ipms/'+ zoneId +'/list/illegal/tempCarRecord/'+ page +'/'+ pageSize;
		console.log(url);
		$.get(url).then(function(data) {
			plus.nativeUI.closeWaiting();
			console.log('获取的报警车辆信息：' + JSON.stringify(data));
...
//					}
				...
				return $.render('tpl/temporary_vehicle_sub_warn_data.html', data);
			} else {
				if(isUp === true) {
					mui.toast('没有更多数据了');
					showTag('.tel-list li');
