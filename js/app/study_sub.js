define(['mui', 'mall'], function(mui, $) {
	...
	mui.plusReady(function() {
		console.log('# studyRefresh plusReady()');
		webviewInit();

		//	获取信息
		window.addEventListener('studyRefresh', function() {
	...
	function pulldownRefresh() {
		page = 0;
		refreshStudy(0, size, false);
	}

	//	上拉加载
	function pullupRefresh() {
		page++;
		refreshStudy(page, size, true);
	}
	...
	function delAnno(id,imgs) {
		...
		var url = urlBase + '/study/delete/'+ id;
	...
	//	获取信息
	function refreshStudy(page, size, isUp) {
		console.log('refreshStudy(' + ',' + page + ',' + size + ',' + isUp + ')');
		plus.nativeUI.showWaiting();
		urlBase = plus.storage.getItem('urlBase');
		var url = urlBase + '/study/search/' + page + '/' + size;
		console.log(url);
		$.get(url).then(function(data) {
			console.log('refreshStudy ' + JSON.stringify(data));
			if(data.errCode == 0){
...
//				};
				console.log(JSON.stringify(data));
				return $.render('tpl/study_sub_data.html', data)
			} else {
				if(isUp === true) {
					mui.toast('没有更多数据了');
					showTag('.anno-all li');
