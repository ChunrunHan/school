define(['mui', 'mall'], function(mui, $) {
	...
	mui.plusReady(function() {
		console.log('black_record_sub plus ready');
		webviewInit();
		
		//	主动下拉刷新
		window.addEventListener('showblack', function() {
			console.log('黑名单车辆刷新');
	...
	function pulldownRefresh() {
		page = 0;
		getCarUpList(page, size, false);
	}
	//	上拉加载
	function pullupRefresh() {
		page++;
		getCarUpList(page, size, true);
	}

	// 获取黑名单车辆列表
	function getCarUpList(page, size, isUp) {
		plus.nativeUI.showWaiting();
		zoneId = plus.storage.getItem('zoneId');
		urlBaseP = plus.storage.getItem('urlBaseP');
		var url = urlBaseP + "/ipms/car/blackWhiteList/1/" + zoneId + "//" + page + "/" + size;
		console.log(url);
		$.get(url).then(function(data) {
			console.log('getCarUpList ' + JSON.stringify(data));
			if(data.errCode == 0) {
				document.querySelector('.search-result').classList.add('mui-hidden');
				return $.render('tpl/black_record_data.html', data);
			} else {
				if(isUp === true) {
					mui.toast('没有更多数据了');
					showTag('.tel-list li');
