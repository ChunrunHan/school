define(['mui', 'mall'], function(mui, $) {
	...
	mui.plusReady(function() {
		console.log('#critica_property_sub plusReady()');
		webviewInit();
//		//	跳转到表扬信息详情
//		$.tapHandler({
//			selector: '.user-logo',
...
//		});
		...
		//	获取社区动态
		window.addEventListener('criticaRefresh', function() {
		...
		//		window.addEventListener('backRefresh', function() {
		//			mui('#refreshList').pullRefresh().pulldownLoading();
		//			var contentWebview = plus.webview.currentWebview();
		//			contentWebview.evalJS("mui('#refreshList').pullRefresh().scrollTo(0,0,100)");
		//		});
		//		window.addEventListener('searchGoods', function(event) {
		//			var categorypirority = plus.storage.getItem('categoryPriority');
		//			var category = categorypirority.split(':');
...
//			}
			...
			$.tapHandler({
				...
				url: 'critica_property_detail.html',
				id: 'infoId',
				event: 'getCritica'
			});
