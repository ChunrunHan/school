define(['mui', 'mall'], function(mui, $) {
	...
	mui.plusReady(function() {
		console.log('userauth_passed plus ready');
		webviewInit();

		//	主动下拉刷新
		window.addEventListener('passed', function() {
			console.log('passed刷新')
	...
	function pulldownRefresh() {
		page = 0;
		getUserPassed(page, size, false);
	}
	//	上拉加载
	function pullupRefresh() {
		page++;
		getUserPassed(page, size, true);
	}

	//	获取已验证的业主列表
	function getUserPassed(page, size, isUp) {
		plus.nativeUI.showWaiting();
		zoneId = plus.storage.getItem('zoneId');
		urlBaseP = plus.storage.getItem('urlBaseP');
		var url = urlBaseP + '/admin/validation/info/search/1/'+ zoneId +'///'+ page + '/' + size;
		console.log(url);
		$.get(url).then(function(data) {
			...
			if(data.errCode === 0) {
				...
				return $.render('tpl/userauth_passed_data.html', data);
