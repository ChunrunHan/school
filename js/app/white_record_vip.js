define(['mui', 'mall'], function(mui, $) {
	...
	mui.plusReady(function() {
		console.log('#white_record_vip plus ready');
		webviewInit();
		
		//	主动下拉刷新
		window.addEventListener('vip', function() {
			console.log('vip车辆刷新')
	...
	function pulldownRefresh() {
		page = 0;
		getCarVipList(page, size, false);
	}
	//	上拉加载
	function pullupRefresh() {
		page++;
		getCarVipList(page, size, true);
	}

	// 获取vip车辆列表
	function getCarVipList(page, size, isUp) {
		plus.nativeUI.showWaiting();
		zoneId = plus.storage.getItem('zoneId');
		urlBaseP = plus.storage.getItem('urlBaseP');
		var url = urlBaseP + "/ipms/car/blackWhiteList/2/" + zoneId + "//" + page + "/" + size;
		console.log(url);
