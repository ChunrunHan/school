...
//	var delimgArry = []; //	 删除的图片
	mui.plusReady(function() {
		console.log('# workRefresh plusReady()');
		webviewInit();

		//	获取信息
		window.addEventListener('workRefresh', function() {
	...
	function pulldownRefresh() {
		page = 0;
		refreshWork(0, size, false);
	}

	//	上拉加载
	function pullupRefresh() {
		page++;
		refreshWork(page, size, true);
	}
	...
	//	获取信息
	function refreshWork(page, size, isUp) {
		console.log('refreshWork(' + ',' + page + ',' + size + ',' + isUp + ')');
		plus.nativeUI.showWaiting();
		urlBase = plus.storage.getItem('urlBase');
		var url = urlBase + '/work/search/' + page + '/' + size;
		console.log(url);
		$.get(url).then(function(data) {
			console.log('refreshWork ' + JSON.stringify(data));
			if(data.errCode == 0){
